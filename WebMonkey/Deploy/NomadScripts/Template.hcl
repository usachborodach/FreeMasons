job "ReportsMonitoring" {
	...
	group "ReportsMonitoring" {
		...
		task "dataharvester" {
			driver = "docker"
			artifact {
				source = "http://__LOCALEMOIVADDRESS__:5657/share/reports_monitoring_container.tar.gz"
			}	
			artifact {
				source = "http://__LOCALEMOIVADDRESS__:5657/share/reports_monitoring_artifacts.zip"
